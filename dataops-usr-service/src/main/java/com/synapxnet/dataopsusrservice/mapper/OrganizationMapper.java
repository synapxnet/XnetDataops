...
public interface OrganizationMapper {
    ...
    @Select("SELECT t.uid AS tenantUid, t.tenant_name AS tenantName, " +
            ...
            "LEFT JOIN xnet_dataops_sys_department d ON d.uid = m.dept_uid AND d.status = 1 " +
            "LEFT JOIN xnet_dataops_sys_team tm ON tm.uid = m.team_uid AND tm.status = 1 " +
     ...
     * @return 有效数据访问成员关系数量
     */
    @Select("SELECT COUNT(*) FROM xnet_dataops_usr_user u " +
            "JOIN xnet_dataops_usr_organization_membership m ON m.user_id = u.id " +
            "WHERE u.phone = #{phone} AND u.status = 'active' " +
